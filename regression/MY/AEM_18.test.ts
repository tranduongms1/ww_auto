...
const title = 'AEM Estore - Add Product in Add-on page from IM PD page';
const tag = ['@Core'];

test.use({ site: 'MY' });
test.use({ product: product.any.hasPD.hasAddOn.canBuy });

test(title, { tag }, async ({ context, page, pd, cart, capture, errors }) => {
    const product = context.product!;

    // 1. Access AEM IM PD page
    pd.product = product;
    ...
    // 4. Verify that customer can go to Cart page directly
