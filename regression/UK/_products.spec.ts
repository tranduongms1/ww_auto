    ...
    test.describe(() => {
        ...
        test(sku, async ({ context, page, bc, pd, cart }, testInfo) => {
            ...
            if (p.bcURL) {
                ...
                await pd.tradeInSection.isVisible().then(v => v ?
                ...
                await pd.tradeUpSection.isVisible().then(v => v ? (p.tradeUp = true) : (typeof p.tradeUp === 'boolean' && (p.tradeUp = false))).catch(noop);
