...
const title = 'SMB registration - Auto Approved';
const tag = ['@Tier2'];
...
test(title, { tag }, async ({ context, page, sso }) => {
    ...
    await test.step('SMB registration process', async () => {
        ...
        await page.getByRole('textbox', { name: 'companyRegistrationNumber' }).fill('03086621');
    ...
    await expect(page.getByRole('heading', { name: 'Registration Approved' }), '"Registration Approved" heading is not visible').toBeVisible({ timeout: 30000 });
});
