...
test(title, { tag }, async ({ context, page, accounts, sso, cart }) => {
    ...
    await expect.soft(cart.empty.signInBtn, 'Sign in button on cart empty page is not visible').toBeVisible();
    await expect.soft(cart.empty.signInBtn, 'Sign in button on cart empty page is not enabled').toBeEnabled();
    ...
    accounts.push({
        ...
        password,
        emailAccount: {
            url: 'https://moakt.com/', account: email, password,
